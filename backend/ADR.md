...
**Decision**
Chose option (c). We inject `Map<String, RoutingStrategy> strategies` into the `RoutingService`. The active strategy name is injected via `@Value("${routing.active-strategy}")`. To add `ZoneAffinityStrategy` in Sprint 2, we just create the class and annotate it with `@Component("zoneAffinity")`. No existing classes or selection logic need to be touched.
**Tradeoffs accepted**
Since Spring populates the map automatically by bean name, it's slightly less explicit than a manual factory. Also, if the `routing.active-strategy` property is misspelled, it will throw a runtime exception rather than failing at compile-time. We mitigated this by adding a validation check in the `RoutingService` constructor.
